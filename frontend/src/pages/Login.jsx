  ...
  async function handleSubmit(e) {
    ...
    try {
      if (tab === 'login') {
        ...
        const { register } = await import('../context/AuthContext').then(m => ({ register: null }))
        // For register use api directly
        const api = (await import('../api/client')).default
        const { data } = await api.post('/auth/register', { name: form.name, email: form.email, password: form.password, company: form.company, role: form.role })
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 bg-gradient-to-br from-orange-500 to-orange-600 p-12 text-white">
        <div className="flex items-center gap-3">
          ...
          <span className="text-2xl font-black tracking-tight">sitePilot</span>
        </div>
        <div>
          <h1 className="text-5xl font-black leading-tight mb-4">
            Manage your construction sites<br />from anywhere.
          </h1>
          <p className="text-orange-100 text-lg">
            Track projects, daily reports, materials and progress photos — all in one place.
          </p>
        ...
        <p className="text-sm text-orange-200">© 2025 sitePilot. Built for field engineers.</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-gray-50">
        <div className="w-full max-w-md">
          ...
          <div className="card shadow-lg">
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
              {['login', 'register'].map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${tab === t ? 'bg-white shadow text-orange-600' : 'text-gray-500 hover:text-gray-700'}`}>
            ...
            <p className="text-center text-xs text-gray-400 mt-2">Uses ravi@sitepilot.com / password123</p>
          </div>
        </div>
      </div>
    </div>
